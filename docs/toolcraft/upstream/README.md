...
This app modifies 29 framework files and adds 4 more under `src/toolcraft/`.
...
`src/toolcraft/` exactly: 33 of 33 files hash-identical, 0 differing. The
...
Two of the 33 are not timeline work and should be judged separately:
`ui/components/primitives/slider/slider-parts.tsx`, which makes a slider state
its own range so an orientation proof can read it, and the export files behind
the AV1 fallback.
