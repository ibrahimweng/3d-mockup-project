...
- Risk: The integrity check reports three modified framework-owned files: `index.html` (a favicon), `e2e/app-persistence.spec.ts` (a longer timeout for a proof that genuinely takes minutes), and `ui/components/primitives/slider/slider-parts.tsx` (a slider stating its own range, which the orientation proof reads). Each was a deliberate choice recorded where it was made, and each properly belongs upstream in the source runtime rather than in this generated folder.
