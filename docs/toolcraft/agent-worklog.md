...
- Risk: A spin angle does not draw the same picture whatever preceded it. Reaching 186 degrees from 90, from 0 and from -186 gives three different frames, reproducibly and across separate runs, with the control reading the same value, the playhead at the same time and the same number of keyframes on the row. Screenshots at 1080x1350 show about a tenth of the pixels differing: mostly small shading deltas averaging 3.6 of 255, with a worst pixel of 149, which reads as lighting or reflection rather than geometry or framing. Ruled out as a measurement artifact by settling far harder than any test does — a ten-second wait plus three consecutive agreeing reads two seconds apart gives the same three answers. `setSpin` writes an absolute rotation and `frameShadow` sizes itself on the bounding sphere captured at build, so neither is the accumulator; what carries the state has not been found yet. The keyframe proof controls for it by driving both its passes along the same route, so that proof is about keyframes rather than about this, and the defect stands on its own.
- Risk: Shrinking the artboard stops the animation. With the canvas at 540x675 or 320x400 the timeline clock still advances and wraps, but the rendered frame freezes: six screenshots across twelve seconds of playback differ by about a tenth of a percent, which is noise. At the authored 1080x1350 the same playthrough draws a different frame every sample. It was found while trying to make playback cheap enough for a test to observe, and it is not understood; the timeline proof runs at the authored size instead and pays for the slowness with a longer timeout.
