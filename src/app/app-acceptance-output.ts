...
import { exportAcceptance } from "./app-acceptance-export";
...
export const outputAcceptance: readonly ToolcraftComponentAcceptance[] = [
  ...
  {
    automated: true,
    automatedTestName: "tilt leans the device without turning it",
    browser: true,
    browserTestName: "browser: tilt leans the device and spin still turns it upright",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "Moving Tilt pitches the device forward or back about the axis running across the frame. It is a different picture from any Spin setting: spin never changes how much of the top or bottom of the device you can see, and tilt is exactly that change. With a tilt in force, spin still sweeps the device around the room's vertical rather than keeping the lean pointed at the camera.",
    fixture: "the default device in the default studio",
    id: "device.tilt.lean",
    kind: "control",
    target: "device.tilt",
    timelineCoverage: "keyframes",
    userAction: "Drag the Tilt slider from 0\u00b0 to 45\u00b0, then set Spin to 90\u00b0.",
  },
  {
    automated: true,
    automatedTestName: "roll cants the device sideways",
    browser: true,
    browserTestName: "browser: roll cants the device and its reflection cants with it",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "Moving Roll rotates the device in the plane of the frame, the way a hand holds a phone off square. The reflection in the floor cants the opposite way, as a reflection does, rather than staying upright or leaning the same way as the device.",
    fixture: "the default device on a reflective floor",
    id: "device.roll.cant",
    kind: "control",
    target: "device.roll",
    timelineCoverage: "keyframes",
    userAction: "Raise Reflection, then drag the Roll slider from 0\u00b0 to 30\u00b0.",
  },
  {
    automated: true,
    automatedTestName: "position x slides the device across the set",
    browser: true,
    browserTestName: "browser: position x moves the device across the frame and back",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "Raising Position X moves the device towards one side of the frame and lowering it moves it to the other, by the same distance for the same number. The camera does not move with it: the shadow travels across the floor with the device rather than staying under the middle of the frame. Returning to 0 gives the picture it started with.",
    fixture: "the default device in the default studio",
    id: "device.position-x.slide",
    kind: "control",
    target: "device.positionX",
    timelineCoverage: "keyframes",
    userAction: "Drag Position X to 50%, then to -50%, then back to 0.",
  },
  {
    automated: true,
    automatedTestName: "position y lifts the device off the floor",
    browser: true,
    browserTestName: "browser: position y lifts the device and its shadow stays on the floor",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "Raising Position Y lifts the device off the surface it was standing on. The shadow stays on the floor where the floor is rather than rising with the device, which is what makes the lift read as the device leaving the ground instead of the whole set moving.",
    fixture: "the default device in the default studio",
    id: "device.position-y.lift",
    kind: "control",
    target: "device.positionY",
    timelineCoverage: "keyframes",
    userAction: "Drag Position Y to 50%, then back to 0.",
  },
  {
    automated: true,
    automatedTestName: "position z moves the device towards the camera",
    browser: true,
    browserTestName: "browser: position z moves the device through the set and changes its size in frame",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "Raising Position Z brings the device towards the camera, so it covers more of the frame; lowering it sends the device back into the set, where it covers less. This is a different picture from Zoom at the same apparent size, because moving the device changes how its depth converges and zoom does not.",
    fixture: "the default device in the default studio",
    id: "device.position-z.depth",
    kind: "control",
    target: "device.positionZ",
    timelineCoverage: "keyframes",
    userAction: "Drag Position Z to 60%, then to -60%.",
  },
  {
    automated: true,
    automatedTestName: "scale resizes the device from its feet",
    browser: true,
    browserTestName: "browser: scale resizes the device and leaves it standing on the surface",
    componentType: "slider",
    evidence: "product-output",
    expectedObservable:
      "Raising Scale makes the device larger in frame and lowering it makes the device smaller, without the camera moving. It grows upward from where it stands rather than outward from its middle: the bottom of the device stays on the surface at every size, so it never sinks through the floor or floats above it.",
    fixture: "the default device in the default studio",
    id: "device.scale.resize",
    kind: "control",
    target: "device.scale",
    timelineCoverage: "keyframes",
    userAction: "Drag Scale to 200%, then to 50%, then back to 100%.",
  },
  {
    automated: true,
