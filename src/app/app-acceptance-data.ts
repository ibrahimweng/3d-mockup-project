...
export const appTransferMode: ToolcraftTransferMode = {
  ...
  referenceFeatureInventory: [
    ...
    },
    {
      ...
      toolcraftMapping:
        "Same `camera.orbit` orientationGizmo target, the same pose writes through runtime dispatch with one history group per gesture, and the same dirty-flag render loop. What differs is which pointer claims the rotation: `src/app/view-orbit.ts` takes any plain primary drag rather than only one that hits the device, and the board moves on the middle button rather than on a miss. The pose is also keyframeable, which the reference had no timeline for: once the camera is keyed the same drag writes a keyframe at the playhead instead of the value, still one history group per gesture, and the timeline carries the pose round the product rather than through it.",
